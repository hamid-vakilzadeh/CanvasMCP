...
class Conversation(TypedDict):
    ...
    properties: Optional[
        List[str]
    ]  # Can contain "last_author", "attachments", "media_objects"
...
def list_conversations(
    ...
    if all_pages:
        return _get_all_pages(
            base_url, access_token, "GET", "/api/v1/conversations", params=params
        )
    else:
...
def create_conversation(
    ...
    if mode not in valid_modes:
        raise ValueError(
            f"Invalid mode '{mode}'. Allowed values: {', '.join(sorted(valid_modes))}"
        )
    ...
    response = _make_request(
        base_url, access_token, "POST", "/api/v1/conversations", data=data
    )
    return response.json()
...
def get_running_batches(
    ...
    """
    response = _make_request(
        base_url, access_token, "GET", "/api/v1/conversations/batches"
    )
    return response.json()
...
def mark_all_as_read(
    ...
    """
    response = _make_request(
        base_url, access_token, "POST", "/api/v1/conversations/mark_all_as_read"
    )
    return response.json()
...
def delete_conversation(
    ...
    """
    response = _make_request(
        base_url, access_token, "DELETE", f"/api/v1/conversations/{conversation_id}"
    )
    return response.json()
...
def add_recipients(
    ...
    response = _make_request(
        ...
        access_token,
        "POST",
        f"/api/v1/conversations/{conversation_id}/add_recipients",
        data=data,
    )
    return response.json()
...
def batch_update_conversations(
    ...
    response = _make_request(
        base_url, access_token, "PUT", "/api/v1/conversations", data=data
    )
    return response.json()
...
def unread_count(
    ...
    """
    response = _make_request(
        base_url, access_token, "GET", "/api/v1/conversations/unread_count"
    )
    return response.json()
