...
class QuizSubmissionsAPI(CanvasAPIBase):
    """Canvas LMS Quiz Submissions API client for accessing quiz submissions."""

    def __init__(self, access_token: str = None, base_url: str = None):
        """
        Initialize the Canvas Quiz Submissions API client.
    ...
    def update_quiz_submission(
        ...
        for i, submission in enumerate(quiz_submissions):
            ...
            if "fudge_points" in submission and submission["fudge_points"] is not None:
                data[f"quiz_submissions[{i}][fudge_points]"] = submission[
                    "fudge_points"
                ]
            ...
            if "questions" in submission:
                ...
                for question_id, question_data in questions.items():
                    if "score" in question_data and question_data["score"] is not None:
                        data[
                            f"quiz_submissions[{i}][questions][{question_id}][score]"
                        ] = question_data["score"]
                    if (
                        "comment" in question_data
                        and question_data["comment"] is not None
                    ):
                        data[
...
def get_quiz_submissions():
    ...
    return QuizSubmissionsAPI(access_token, url)

class _LazyQuizSubmissionsAPI:
    def __getattr__(self, name):
        return getattr(get_quiz_submissions(), name)

quiz_submissions = _LazyQuizSubmissionsAPI()
