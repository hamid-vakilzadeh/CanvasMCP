...
class QuizQuestionGroupsAPI(CanvasAPIBase):
    """Canvas LMS Quiz Question Groups API client for managing quiz question groups."""

    def __init__(self, access_token: str = None, base_url: str = None):
        """
        Initialize the Quiz Question Groups API client.
    ...
    def get_quiz_group(
        ...
        response = self._make_request(
            "GET", f"/api/v1/courses/{course_id}/quizzes/{quiz_id}/groups/{group_id}"
        )
    ...
    def create_quiz_group(
        ...
        response = self._make_request(
            "POST", f"/api/v1/courses/{course_id}/quizzes/{quiz_id}/groups", data=data
        )
        return response.json()
    ...
    def update_quiz_group(
        ...
        response = self._make_request(
            "PUT", f"/api/v1/courses/{course_id}/quizzes/{quiz_id}/groups/{group_id}", data=data
        )
        return response.json()
    ...
    def delete_quiz_group(
        ...
        """
        self._make_request(
            "DELETE", f"/api/v1/courses/{course_id}/quizzes/{quiz_id}/groups/{group_id}"
        )
        return None
    ...
    def reorder_quiz_group_questions(
        ...
        for i, item in enumerate(order):
            ...
            if "type" in item and item["type"] != "question":
                raise ValueError(f"Order item type must be 'question', got '{item['type']}'")
