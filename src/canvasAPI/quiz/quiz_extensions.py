...
class QuizExtensionsAPI(CanvasAPIBase):
    """Canvas LMS Quiz Extensions API client for setting extensions on student quiz submissions."""

    def __init__(self, access_token: str = None, base_url: str = None):
        """
        Initialize the Canvas Quiz Extensions API client.
    ...
    def set_quiz_extensions(
        ...
        for i, extension in enumerate(quiz_extensions):
            ...
            if "extra_attempts" in extension:
                data[f"quiz_extensions[{i}][extra_attempts]"] = extension[
                    "extra_attempts"
                ]
...
def get_quiz_extensions():
    from ..base import access_token, url
    return QuizExtensionsAPI(access_token, url)

class _LazyQuizExtensionsAPI:
    def __getattr__(self, name):
        return getattr(get_quiz_extensions(), name)

quiz_extensions = _LazyQuizExtensionsAPI()
