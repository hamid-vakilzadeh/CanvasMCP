...
def list_assignment_groups(
    ...
    if all_pages:
        return _get_all_pages(
            base_url, access_token, "GET", f"/api/v1/courses/{course_id}/assignment_groups", params=params
        )
    else:
...
def create_assignment_group(
    ...
    response = _make_request(
        base_url, access_token, "POST", f"/api/v1/courses/{course_id}/assignment_groups", data=data
    )
    return response.json()
...
def delete_assignment_group(
    ...
    Example:
        ...
        result = delete_assignment_group(base_url, access_token, 123, 456, move_assignments_to=789)

        # Delete group and all its assignments
        result = delete_assignment_group(base_url, access_token, 123, 456)
    """
    params = {}
