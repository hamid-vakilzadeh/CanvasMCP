...
class SessionManager:
    ...
    def __init__(
        self, cleanup_interval_minutes: int = 5, session_timeout_minutes: int = 15
    ):
        """
    ...
    def start_cleanup_task(self) -> None:
        ...
        if not self._running:
            ...
            logger.info(
                f"Session cleanup task started (interval: {self.cleanup_interval}s, timeout: {self.session_timeout}m)"
            )
    ...
    def create_session(self, session_id: str, base_url: str, access_token: str) -> None:
        ...
        """
        with self._lock:
            session_data = SessionData(base_url=base_url, access_token=access_token)
